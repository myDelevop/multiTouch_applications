...
public class MTRadialPoly extends MTPolygon {
	...
	private MTColor selectedColor = new MTColor(MTColor.GREEN);

	private MTColor unSelectedColor = new MTColor(64, 163, 251, 180);
	...
	public MTRadialPoly(final PApplet pApplet, final Vertex[] vertices,
		...
		font = FontManager.getInstance().createFont(pApplet, "arial.ttf",
				16, // Font size
				new MTColor(MTColor.WHITE), // Font fill color
				new MTColor(MTColor.WHITE), // Font fill color
				true); // Anti-alias
...
*/							}
	...
	public MTColor getUnSelectedColor() {
		if (unSelectedColor == null)
			unSelectedColor = new MTColor(MTColor.GREY);
	 ...
	 * @param subMenuActive
	 */
	public void setSubMenuActive(final boolean subMenuActive) {
	 ...
	 */
	public List<MTRadialPoly> getSubMenuItems() {
	...
	protected void removeAllMenuItems() {
		for (final MTRadialPoly item : this.subMenuItems) {
			...
			System.out.println("Rimuovo altri submenu");
	 ...
	 * @param menuText
	 */
	private void createMenuItem(final PApplet pApplet, final IFont font, String menuText) {
		...
		mtTextArea = new MTTextArea(pApplet);
		mtTextArea.setName(menuText);
		if (menuText.length() > 20){
			menuText = menuText.substring(0, 18);
			menuText = menuText + "...";
		}
		...
		mtTextArea.setGestureAllowance(TapProcessor.class, true);
		mtTextArea.setGestureAllowance(ScaleProcessor.class, false);
		mtTextArea.setGestureAllowance(RotateProcessor.class, false);
		//System.out.println(getUnSelectedColor());
		//font.setFillColor(new MTColor(Math.abs(getFillColor().getR()-255),Math.abs(getFillColor().getG()-255), Math.abs(getFillColor().getB()-255)));
	...
	public MTTextArea getMtTextArea() {
		return mtTextArea;
	}

	public void setMtTextArea(MTTextArea mtTextArea) {
		this.mtTextArea = mtTextArea;
	}

	public void addListner(IMTInputEventListener inputEventListener) {
