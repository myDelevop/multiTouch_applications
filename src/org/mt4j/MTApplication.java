 ...
 */
public abstract class MTApplication extends PApplet implements Serializable {
	/** The Constant logger. */
	...
	private static void getSettingsFromFile(){
		 ...
		 try {
			 ...
			 */
			 ...
			 logger.error("Error while loading Settings.txt from " + MT4jSettings.getInstance().getDefaultSettingsPath() + ". Using defaults.");
		 }
		 settingsLoadedFromFile = true;
	}
...
//					"MT4j.gif");
			...
			int offset = 50;
			//modificare qui per dimensione e posizione finestra
			frame.setBounds(0, -1*offset, frame.getWidth(), frame.getHeight()+offset*2);
			//frame.setSize(1980, 1100);
...
//	    	gl.glDisable(GL.GL_LINE_SMOOTH);
	...
	public void draw(){
		try {
			...
			e.printStackTrace();
			System.out.println("Modifica fatta in MTApplication.java nel metodo draw");
		}
