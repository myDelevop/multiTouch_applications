...
//		}
	...
	public void removeComponentChildren(MTComponent root, int livello) {
	     MTComponent[] children = root.getChildren();
	     for(MTComponent c:children) {
	         if (!(c instanceof MTOverlayContainer)) {
	             c.removeComponentChildren(c, livello++);
	             root.removeChild(c);
	         } else {
	                System.err.println(livello);
	                System.out.println("NON Elimino: "+c.getClass()+ " che ha come padre " + c.getParent().getClass());
	         }
	     }
	 }
