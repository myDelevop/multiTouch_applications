...
/**
 * The Class ChooseBackgroundScene.
 */
public class ChooseBackgroundScene extends AbstractScene {
	...
	/**
	 * Instantiates a new choose background scene.
	 *
	 * @param mtApplication the mt application
	 * @param name the name
	 */
	public ChooseBackgroundScene(final MTApplication mtApplication, 
	        final String name) {
					...
					if (posEvt.getId() != AbstractCursorInputEvt.INPUT_ENDED) {
						registerPreDrawAction(new IPreDrawAction() {
							public void processAction() {
								...
								Vector3D direction = 
								        pos.getSubtracted(lastDrawnPoint);
								float distance = direction.length();
								direction.normalizeLocal();
								direction.scaleLocal(scaledStepDistance);

								float howManySteps =
								        distance / scaledStepDistance;
								...
								Vector3D currentPos 
								= new Vector3D(lastDrawnPoint);
								for (int i = 0; i < stepsToTake; i++) {
								    currentPos.addLocal(direction);
									Vector3D diff = currentPos.getSubtracted(
									        localBrushCenter);
									mtApp.pushMatrix();
