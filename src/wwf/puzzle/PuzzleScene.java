...
public class PuzzleScene extends AbstractScene{
	...
	/** The images path. */
	private String imagesPath = "data"+MTApplication.separator+"puzzle"+MTApplication.separator;
	private String backgroundPath = "src"+MTApplication.separator+"wwf"+MTApplication.separator+"puzzle"+MTApplication.separator+"data"+MTApplication.separator;
	...
	public PuzzleScene(MTApplication mtApplication, String name) {
		...
		MTBackgroundImage background = new MTBackgroundImage(mtApplication, mtApplication.loadImage(backgroundPath + "webtreats_wood-pattern1-512d.jpg") , true);
		...
		IFont font = FontManager.getInstance().createFont(mtApplication, "SansSerif", 40, MTColor.WHITE, false);
		
		//New Puzzle button
		MTRoundRectangle r = getRoundRectWithText(0, 0, 360, 90, "Apri puzzle", font);
		...
		r.setFillColor(new MTColor(0, 0, 0, 100));
		...
		//Image list
		LinkedList<String> imagesNames = getPictureFiles(new File(imagesPath));
		float cellWidth = 500;
		float cellHeight = 80;
		MTColor cellFillColor = new MTColor(MTColor.BLACK);
		MTColor cellPressedFillColor = new MTColor(new MTColor(105,105,105));
		list = new MTList(getMTApplication(), r.getWidthXY(TransformSpace.GLOBAL) + 5, 0, cellWidth+2, imagesNames.size()* cellHeight + imagesNames.size()*3);
		list.setNoFill(true);
...
//		list.setPositionGlobal(Vector3D.ZERO_VECTOR);
		list.setVisible(false);		
		Iterator<String> itImagesNames = imagesNames.iterator();
		while (itImagesNames.hasNext()) {
			String imageName = (String) itImagesNames.next();
	...
	public LinkedList<String> getPictureFiles(File dir) {
		File f_list[] = dir.listFiles();
		LinkedList<String> fotoFiles = new LinkedList<String>();
		FileNameExtensionFilter image_filter = new FileNameExtensionFilter("JPEG file", "jpeg", "jpg");
		int j = 0;

		while (j < f_list.length) {
			if (f_list[j].isDirectory()) {
				getPictureFiles(f_list[j]);
			} else if (f_list[j].isFile()) {
				if (image_filter.accept(f_list[j])) {
					System.out.println("Path " + f_list[j]);
					// System.out.println(""+files.length);
					fotoFiles.add(f_list[j].getName());
				}
			}
			j++;
		}
...
//											sh.tweenTranslateTo(x, y, 0, 400, 0f, 1.0f);
            ...
            //decommentare per distruibuire le tessere con rotazione
